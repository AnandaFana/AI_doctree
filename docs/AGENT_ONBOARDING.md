...
`context` 按显式路径读取目标，不受页面折叠限制；默认只包含目标正文、祖先职责和直接孩子入口。核对 `usage` / `truncation`，需要更多正文时显式 `--body-scope all` 或按孩子路径单独读取；不能把精简上下文当作已经读过祖先的全部约束。树图仍有展示预算，不假装页面包含整个项目。
...
如果项目已有 doctree-review.json，收尾时执行 `python -X utf8 .doctree/manage.py review check`，按实际差异更新说明或记录无需更新及理由，再逐级判断上级影响。用户已要求变化提醒且 Git 历史可用时可运行 review init 启用；否则作为可选后续步骤，不阻塞普通接入。review ack 的参数见命令 --help，token 必须来自重新检查的当前输入。

...
详细指令的仓库原文见 [Agent 首次接入指令](AGENT_ONBOARDING.md)。对已安装旧版本的项目，先在工具仓库运行 `git pull --ff-only`，再重新执行第 2 步即可升级，安装器保留原有历史和备份。已经建立的业务节点无需重新生成；Git 维护检查仍需按需启用。
...
树图仍按展示范围折叠；`context` 已改为路径直达，不受网页节点数与批次折叠影响。默认只输出目标正文及祖先、孩子的摘要入口；输出预算和截断标记需要核对，需要更多正文时使用 `--body-scope all` 或读取具体节点。
...
需要发现忘记更新的说明时，可按 [维护指南](MAINTENANCE.md) 选择启用 Git 收尾检查。它不是安装的前置条件；没有 Git 时继续使用普通导航。

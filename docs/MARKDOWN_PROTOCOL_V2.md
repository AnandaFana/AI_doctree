...
覆盖检查独立于网页的深度与批量产物折叠。`inventory_complete` 表示所选目录盘点完成，`protocol_discovery.complete` 表示全局既有节点发现完成；两者都成功时 `scope_complete` 才为真，`scope_covered` 还要求这些目录具有节点说明；`project_complete` 还要求没有延后检查的目录。预算或读取错误单独报告，不能用浅层检查冒充全仓完成。未选范围是待评估，不是错误。

可用 `--policy` JSON 保存可复用范围：`{"schema":1,"max_depth":1,"exclude_dirs":[]}`。希望跨人或跨 Agent 共享此约定时，将它与项目 Markdown 一起保存到 Git 可见位置；`.doctree/` 中不应保存唯一的范围约定。治理 `exclude_dirs` 只排除新增说明范围，不能绕过全局协议发现。预检会单独列出全局发现错误；例如被排除归档中的非 UTF-8 Markdown 仍可能阻断同步，但不会再出现预检通过后才报错的情况。不会自动改编码或丢弃旧节点；如何处理来源或重新选择项目根由用户决定。范围外已接入的深层节点不会因为本次范围较浅而失去导航。
...
已接入节点改名或移动目录时保留 `id`，再同步相对链接。同步不得自动更换已有 ID。新子节点默认 ID 使用项目 ID 与规范相对路径的哈希（包括路径分隔边界），`annotate`、选择文件和 `cover` 共用同一算法；`a/b` 与 `a-b` 不会再因替换分隔符而碰撞。根默认保留项目 ID。已有 ID 不重算、不迁移。
...
解析后的语义版本排除合法的生成导航区，保留身份、职责和人工正文。纯导航修复不应成为新的业务来源变化。原始文件字节哈希仍用于并发检查和写入审计。旧扫描器的算法标识已升级为 `doctree-scan-v3`（动态交付证据核对和完整扫描保护），不能把旧指纹当作新算法计算结果。Markdown 格式仍为 schema 2。
...
`context` 直接检查目标目录、路径上的祖先和直接孩子，不先构建界面树。默认目标正文最多 8000 字符，祖先只职责与入口，孩子只标题、职责与路径；最多 8 个祖先、20 个孩子，总输出最多 24000 个字符（按缩进 JSON 及末尾换行计数）。`--body-scope none/target/all` 选择正文范围，`--max-chars` 与 `--max-children` 控制输出。API 另有来源读取预算，`usage`、`truncation` 和文档截断标记说明限制。需要祖先完整约束时显式扩展或按节点读取；精简上下文不代表读过全部约定。`tree` 输出可重建 JSON，没有写入第一版 `state.json` 的依赖。
...
可选的 `review` 命令以 Git 基线和共享 JSON 记录变化、核对理由及直接父级吸收的子 token。网页加载/刷新时显示待核对提示；它不自动总结或监听，也不改变 README 的语义版本定义。业务文件版本由独立维护检查跟踪，文档核对依然由人或 Agent 声明。详见 [维护约定](MAINTENANCE.md)。

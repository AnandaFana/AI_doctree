...
- 如果项目已经启用 `doctree-review.json`，收尾运行 `review check`；Agent 阅读差异后记录更新或无需更新的理由，只有需要才提示直接父节点。不依赖工具自动总结或业务验收。
'''
...
8. `context --directory 路径` 默认输出目标正文、祖先职责及孩子入口，有 24000 字符总预算；核对截断标记，需要更多正文时加 `--body-scope all` 或单独读取孩子。
9. 可选 `review init` 建立 Git 比较基线，`review check` 发现相关目录变化。根据实际差异更新说明，或用 `review ack --help` 查看如何记录无需更新的理由及父级影响。共享记录 `doctree-review.json` 随项目保留；这里的核对只是人或 Agent 的文档声明，不是业务验收。
...
def handler_for(projects, port, web=None):
    from . import foldertree, maintenance
        ...
        def do_GET(self):
            ...
            try:
                if url.path == "/api/health":
                    ...
                    self.send(maintenance.attach(foldertree.build_tree(projects), projects))
...
def main(argv=None, *, project_root=None) -> int:
    from . import coverage, foldertree, markdown_protocol, maintenance
    ...
    context.add_argument("--body-scope", choices=("target", "none", "all"), default="target")
    context.add_argument("--max-chars", type=int, default=24000)
    context.add_argument("--max-children", type=int, default=20)
    review = sub.add_parser("review", help="显式检查 Git 变化，记录文档核对；不自动改写 README")
    review_sub = review.add_subparsers(dest="review_command", required=True)
    review_init = review_sub.add_parser("init")
    review_init.add_argument("--base", default="HEAD")
    review_check = review_sub.add_parser("check")
    review_check.add_argument("--check", action="store_true", help="待核对或未归属变化时退出 1")
    review_ack = review_sub.add_parser("ack")
    review_ack.add_argument("--node", required=True)
    review_ack.add_argument("--token", required=True)
    review_ack.add_argument("--decision", choices=("updated", "no_change"), required=True)
    review_ack.add_argument("--reason", required=True)
    review_ack.add_argument("--reviewer", required=True)
    review_ack.add_argument("--parent-impact", choices=("needed", "not_needed"), required=True)
    review_ack.add_argument("--parent-reason", required=True)
    ...
    try:
        ...
        if args.command == "review":
            if args.review_command == "init":
                result = maintenance.initialize(root, base=args.base)
            elif args.review_command == "check":
                result = maintenance.check(root)
                print(json.dumps(result, ensure_ascii=False, indent=2))
                return 2 if not result['complete'] else int(args.check and bool(result['pending'] or result['unowned_changes']))
            else:
                result = maintenance.acknowledge(root, node_id=args.node, token=args.token,
                    decision=args.decision, reason=args.reason, reviewer=args.reviewer,
                    parent_impact=args.parent_impact, parent_reason=args.parent_reason)
            ...
            result = foldertree.context([project], project["id"], args.directory,
                body_scope=args.body_scope, max_chars=args.max_chars, max_children=args.max_children)
        else:
            result = maintenance.attach(foldertree.build_tree([project]), [project])
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
